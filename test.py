...
for pose_id in range(0, 11):
	...
	first_image = cv2.imread('/database3/KITTI_odometry/dataset/sequences/' + str(pose_id).zfill(2) + '/image_2/' + '000000.png', 0)
	h, w = first_image.shape
	h, w = float(h), float(w)
	f = open('/database3/KITTI_odometry/dataset/sequences/' + str(pose_id).zfill(2) + '/calib.txt', 'r')
	...
	for img_id in range(pose_file_len):
		img_file_loc = '/database3/KITTI_odometry/dataset/sequences/' + str(pose_id).zfill(2) + '/image_2/' + str(img_id).zfill(6) + '.png'
